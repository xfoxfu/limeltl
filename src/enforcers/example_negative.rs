    ...
    match ty {
        Variable::And(s) => {
            if t == e_max_t {
                ...
                vec![
                    Run(e, t, s) << (Run(e, t, s) & Variable::Or(s) & LeftChild(s, s1)),
                    Run(e, t, s2) << (Run(e, t, s) & Variable::Or(s) & RightChild(s, s2)),
                ]
            }
        }
        Variable::Or(s) => vec![
        ...
        Variable::Eventually(s) => vec![
            (if t < e_max_t {
                ...
                Run(e, t, s1).into()
            }) << (Run(e, t, s) & Always(s) & LeftChild(s, s1)),
        ],
...
pub struct NegativeExampleEnforcer<'a>(Variable, &'a Example);

impl<'a> NegativeExampleEnforcer<'a> {
    /// 构造正例的 Enforcer
...
impl<'a> Enforcer for NegativeExampleEnforcer<'a> {
